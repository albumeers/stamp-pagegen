/*
   Copyright 2012-2026 Jason Drake (jadrake75@gmail.com)
...
public class FontRegistry {
	...
	public void initializeFonts() {
		if (!registered) {
			...
			if (operatingSystem != null && operatingSystem.toLowerCase().contains("windows")) {
				File winSysFonts = new File("C:\\Windows\\Fonts");
				if (winSysFonts.exists() && winSysFonts.isDirectory()) {
					fontDirs.add(winSysFonts);
				}

				String localAppData = System.getenv("LOCALAPPDATA");
				if (localAppData != null && !localAppData.isEmpty()) {
					File winUserFonts = new File(localAppData, "Microsoft\\Windows\\Fonts");
					if (winUserFonts.exists() && winUserFonts.isDirectory()) {
						fontDirs.add(winUserFonts);
					}
				}
			} else {
				String javaFonts = System.getenv("JAVA_FONTS");
				if (javaFonts != null && !javaFonts.isEmpty()) {
					File jf = new File(javaFonts);
			...
			String customPath = getFontPath();
			if (customPath != null && !customPath.isEmpty()) {
				File customDir = new File(customPath);
				if (customDir.exists() && customDir.isDirectory()) {
					fontDirs.add(customDir);
				}
			}
			...
			for (File dir : fontDirs) {
				try {
					...
					if (!registeredPaths.contains(canonicalPath)) {
						registeredPaths.add(canonicalPath);
						FontFactory.registerDirectory(canonicalPath, true);
					}
				} catch (Exception e) {
					logger.log(Level.FINE,
							"Failed to resolve canonical path for font directory: " + dir.getAbsolutePath(), e);
				}
			}
	...
	/**
	 * Retrieves the font mappings from the preferences using the fully qualified
	 * preference node path
	 * 
	 * <pre>
	 * /org/javad/pdf/fonts/mapping
	 * </pre>
	 * 
	...
	protected String getNativeFontPath() {
		...
		try {
			...
			if (clazz != null) {
				...
				if (m != null) {
					Object result = m.invoke(null, true);
					if (result != null) {
			...
			logger.log(Level.FINE,
					"Reflection lookup for sun.font.FontManager failed or not available on current JDK; using fallback path: " + path,
					t);
		}
		logger.info("Calculated system font path is \"" + path + "\"");
		return path;
	}
	...
	public Font getFont(PdfFontDefinition name) {
		...
		if (bean.isSystem()) {
			...
			logger.log(Level.FINE, "Using a mapped font for \"" + name + "\"");
		...
		if (isFontInvalid(f)) {
			logger.info("Unable to find a registered font for \"" + name + "\" using default of Helvetica");
	...
	/**
	 * This method will attempt to find the named font with the specified size and
	 * style using a naming roll-off technique based on some of the naming
	 * conventions observed.
	 * <ul>
	 * <li>If the style is bold and italic, a font with the name
	 * <code>"[name]-bolditalic"</code> will attempted.</li>
	 ...
	 * <li>Finally if none of these styled fonts can be located, the
	 * <code>"[name]"</code> will be tried.</li>
	 * </ul>
	 * 
	 * @param name
	 * @param bean
		...
		if (logger.isLoggable(Level.FINER)) {
			...
			if (bf != null) {
				...
				if (fullName != null && fullName.length >= 1 && fullName[0].length >= 4) {
					logger.finer("The calculated font name is \"" + fullName[0][3] + "\"");
				} else {
					logger.finer("The base font full name was not parseable.");
				}
			} else {
				logger.finer("The base font was null for \"" + name + "\" with style " + bean.getStyle());
			}
		}
		return f;
	}
